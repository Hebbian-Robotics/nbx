// AUTO-GENERATED. Source: schema/netbox-4.5.10.json :: ManufacturerRequest
...
#[derive(Debug, Args)]
pub struct ManufacturerCreateArgs {
    #[arg(long)]
...
#[derive(Debug, Args)]
pub struct ManufacturerUpdateArgs {
...
pub enum ManufacturerAction {
    List(GenericListArgs),
    Get(GetArgs),
    Create(ManufacturerCreateArgs),
    Update(ManufacturerUpdateArgs),
    Delete(DeleteArgs),
...
pub async fn run_manufacturer_action(action: ManufacturerAction, opts: &GlobalOptions) -> NbxResult<()> {
    match action {
        ManufacturerAction::List(args) => generic_list(RESOURCE, args, opts).await,
        ManufacturerAction::Get(args) => generic_get(RESOURCE, args, Some(validate_response), opts).await,
        ManufacturerAction::Create(args) => {
        ...
        ManufacturerAction::Update(args) => {
        ...
        ManufacturerAction::Delete(args) => delete_resource(RESOURCE, args, opts).await,
        ManufacturerAction::BulkUpdate(args) => {
            mutate_collection_with_body(RESOURCE, HttpMethod::Patch, generic_mutation_body(args)?, Some(validate_bulk_response), opts).await
        }
        ManufacturerAction::BulkDelete(args) => bulk_delete_resource(RESOURCE, args, opts).await,
    }
}

async fn create_body(args: ManufacturerCreateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
...
async fn update_body(args: ManufacturerUpdateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    let mut body = optional_json_body(args.data, args.data_file)?
        .unwrap_or_else(|| json!({}));
    let object: &mut Map<String, Value> = body.as_object_mut()
...
pub fn validate_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<crate::generated::types::Manufacturer>(value.clone()).map(|_| ())
}
...
pub fn validate_bulk_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<Vec<crate::generated::types::Manufacturer>>(value.clone()).map(|_| ())
}
