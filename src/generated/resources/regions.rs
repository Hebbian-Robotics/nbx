// AUTO-GENERATED. Source: schema/netbox-4.5.10.json :: WritableRegionRequest
...
pub enum RegionAction {
    ...
    Get(GetArgs),
    Create(RegionCreateArgs),
    Update(RegionUpdateArgs),
    Delete(DeleteArgs),
...
pub async fn run_region_action(action: RegionAction, opts: &GlobalOptions) -> NbxResult<()> {
    match action {
        RegionAction::List(args) => generic_list(RESOURCE, args, opts).await,
        RegionAction::Get(args) => generic_get(RESOURCE, args, Some(validate_response), opts).await,
        RegionAction::Create(args) => {
        ...
        RegionAction::Delete(args) => delete_resource(RESOURCE, args, opts).await,
        RegionAction::BulkUpdate(args) => {
            mutate_collection_with_body(RESOURCE, HttpMethod::Patch, generic_mutation_body(args)?, Some(validate_bulk_response), opts).await
        }
        RegionAction::BulkDelete(args) => bulk_delete_resource(RESOURCE, args, opts).await,
    }
}

async fn create_body(args: RegionCreateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
...
async fn update_body(args: RegionUpdateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    let mut body = optional_json_body(args.data, args.data_file)?
        .unwrap_or_else(|| json!({}));
    let object: &mut Map<String, Value> = body.as_object_mut()
...
pub fn validate_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<crate::generated::types::Region>(value.clone()).map(|_| ())
}
...
pub fn validate_bulk_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<Vec<crate::generated::types::Region>>(value.clone()).map(|_| ())
}
