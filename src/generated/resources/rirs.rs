// AUTO-GENERATED. Source: schema/netbox-4.5.10.json :: RIRRequest
...
pub enum RirAction {
    ...
    Get(GetArgs),
    Create(RirCreateArgs),
    Update(RirUpdateArgs),
    Delete(DeleteArgs),
...
pub async fn run_rir_action(action: RirAction, opts: &GlobalOptions) -> NbxResult<()> {
    match action {
        RirAction::List(args) => generic_list(RESOURCE, args, opts).await,
        RirAction::Get(args) => generic_get(RESOURCE, args, Some(validate_response), opts).await,
        RirAction::Create(args) => {
        ...
        RirAction::Delete(args) => delete_resource(RESOURCE, args, opts).await,
        RirAction::BulkUpdate(args) => {
            mutate_collection_with_body(RESOURCE, HttpMethod::Patch, generic_mutation_body(args)?, Some(validate_bulk_response), opts).await
        }
        RirAction::BulkDelete(args) => bulk_delete_resource(RESOURCE, args, opts).await,
    }
}

async fn create_body(args: RirCreateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    ...
    insert_optional_bool_field(object, "is_private", args.is_private);
...
async fn update_body(args: RirUpdateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    let mut body = optional_json_body(args.data, args.data_file)?
        .unwrap_or_else(|| json!({}));
    let object: &mut Map<String, Value> = body.as_object_mut()
...
pub fn validate_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<crate::generated::types::Rir>(value.clone()).map(|_| ())
}
...
pub fn validate_bulk_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<Vec<crate::generated::types::Rir>>(value.clone()).map(|_| ())
}
