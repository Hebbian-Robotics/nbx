// AUTO-GENERATED. Source: schema/netbox-4.5.10.json :: WritableSiteGroupRequest
...
#[derive(Debug, Args)]
pub struct SiteGroupCreateArgs {
    #[arg(long)]
...
#[derive(Debug, Args)]
pub struct SiteGroupUpdateArgs {
...
pub enum SiteGroupAction {
    List(GenericListArgs),
    Get(GetArgs),
    Create(SiteGroupCreateArgs),
    Update(SiteGroupUpdateArgs),
    Delete(DeleteArgs),
...
pub async fn run_site_group_action(action: SiteGroupAction, opts: &GlobalOptions) -> NbxResult<()> {
    match action {
        SiteGroupAction::List(args) => generic_list(RESOURCE, args, opts).await,
        SiteGroupAction::Get(args) => generic_get(RESOURCE, args, Some(validate_response), opts).await,
        SiteGroupAction::Create(args) => {
        ...
        SiteGroupAction::Delete(args) => delete_resource(RESOURCE, args, opts).await,
        SiteGroupAction::BulkUpdate(args) => {
            mutate_collection_with_body(RESOURCE, HttpMethod::Patch, generic_mutation_body(args)?, Some(validate_bulk_response), opts).await
        }
        SiteGroupAction::BulkDelete(args) => bulk_delete_resource(RESOURCE, args, opts).await,
    }
}

async fn create_body(args: SiteGroupCreateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
...
async fn update_body(args: SiteGroupUpdateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    let mut body = optional_json_body(args.data, args.data_file)?
        .unwrap_or_else(|| json!({}));
    let object: &mut Map<String, Value> = body.as_object_mut()
...
pub fn validate_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<crate::generated::types::SiteGroup>(value.clone()).map(|_| ())
}
...
pub fn validate_bulk_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<Vec<crate::generated::types::SiteGroup>>(value.clone()).map(|_| ())
}
