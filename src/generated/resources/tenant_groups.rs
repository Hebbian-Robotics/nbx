// AUTO-GENERATED. Source: schema/netbox-4.5.10.json :: WritableTenantGroupRequest
...
#[derive(Debug, Args)]
pub struct TenantGroupCreateArgs {
    #[arg(long)]
...
#[derive(Debug, Args)]
pub struct TenantGroupUpdateArgs {
...
pub enum TenantGroupAction {
    List(GenericListArgs),
    Get(GetArgs),
    Create(TenantGroupCreateArgs),
    Update(TenantGroupUpdateArgs),
    Delete(DeleteArgs),
...
pub async fn run_tenant_group_action(action: TenantGroupAction, opts: &GlobalOptions) -> NbxResult<()> {
    match action {
        TenantGroupAction::List(args) => generic_list(RESOURCE, args, opts).await,
        TenantGroupAction::Get(args) => generic_get(RESOURCE, args, Some(validate_response), opts).await,
        TenantGroupAction::Create(args) => {
        ...
        TenantGroupAction::Delete(args) => delete_resource(RESOURCE, args, opts).await,
        TenantGroupAction::BulkUpdate(args) => {
            mutate_collection_with_body(RESOURCE, HttpMethod::Patch, generic_mutation_body(args)?, Some(validate_bulk_response), opts).await
        }
        TenantGroupAction::BulkDelete(args) => bulk_delete_resource(RESOURCE, args, opts).await,
    }
}

async fn create_body(args: TenantGroupCreateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
...
async fn update_body(args: TenantGroupUpdateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    let mut body = optional_json_body(args.data, args.data_file)?
        .unwrap_or_else(|| json!({}));
    let object: &mut Map<String, Value> = body.as_object_mut()
...
pub fn validate_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<crate::generated::types::TenantGroup>(value.clone()).map(|_| ())
}
...
pub fn validate_bulk_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<Vec<crate::generated::types::TenantGroup>>(value.clone()).map(|_| ())
}
