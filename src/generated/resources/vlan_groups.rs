// AUTO-GENERATED. Source: schema/netbox-4.5.10.json :: VLANGroupRequest
...
#[derive(Debug, Args)]
pub struct VlanGroupCreateArgs {
    #[arg(long)]
...
#[derive(Debug, Args)]
pub struct VlanGroupUpdateArgs {
...
pub enum VlanGroupAction {
    List(GenericListArgs),
    Get(GetArgs),
    Create(VlanGroupCreateArgs),
    Update(VlanGroupUpdateArgs),
    Delete(DeleteArgs),
...
pub async fn run_vlan_group_action(action: VlanGroupAction, opts: &GlobalOptions) -> NbxResult<()> {
    match action {
        VlanGroupAction::List(args) => generic_list(RESOURCE, args, opts).await,
        VlanGroupAction::Get(args) => generic_get(RESOURCE, args, Some(validate_response), opts).await,
        VlanGroupAction::Create(args) => {
        ...
        VlanGroupAction::Delete(args) => delete_resource(RESOURCE, args, opts).await,
        VlanGroupAction::BulkUpdate(args) => {
            mutate_collection_with_body(RESOURCE, HttpMethod::Patch, generic_mutation_body(args)?, Some(validate_bulk_response), opts).await
        }
        VlanGroupAction::BulkDelete(args) => bulk_delete_resource(RESOURCE, args, opts).await,
    }
}

async fn create_body(args: VlanGroupCreateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
...
async fn update_body(args: VlanGroupUpdateArgs, opts: &GlobalOptions) -> NbxResult<Value> {
    let _ = opts;
    let mut body = optional_json_body(args.data, args.data_file)?
        .unwrap_or_else(|| json!({}));
    let object: &mut Map<String, Value> = body.as_object_mut()
...
pub fn validate_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<crate::generated::types::VlanGroup>(value.clone()).map(|_| ())
}
...
pub fn validate_bulk_response(value: &Value) -> Result<(), serde_json::Error> {
    serde_json::from_value::<Vec<crate::generated::types::VlanGroup>>(value.clone()).map(|_| ())
}
